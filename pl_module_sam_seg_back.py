...
class SamSeg(LightningModule):
    def __init__(
        ...
        if metrics is not None:
            ...
            self.test_metrics = metrics.clone(postfix='final_test/')
        ...
        self.result_file = f"sam_extra_monuseg_{current_time}.txt"
        ...
        self.best_model_path = "total_model_early_stop"  # 保存最佳模型的路径
        self.best_validation_mask_dir = "best_validation_masks_ph2"
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=None):
        ...
        if not self.trainer.sanity_checking:
            ...
            # 直接更新验证指标
            self.valid_metrics.update(mask_cls_pred, gt_masks)
    ...
    def save_validation_masks(self):
        ...
        for batch_idx, mask_cls_pred in self.validation_masks_cache:
            for i, mask in enumerate(mask_cls_pred):
                ...
                # 保存为灰度图像
                save_path = os.path.join(save_dir, f"val_mask_{batch_idx}_{i}.png")
