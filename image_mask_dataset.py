...
class GeneralDataModule(LightningDataModule):
    ...
    def worker_init_fn(self, worker_id):
        """
        这个方法将在每个 worker 启动时调用，用于初始化每个 worker 的随机种子
        """
        # 获取 worker_seed，并确保它是整数
        worker_seed = self.worker_seed if hasattr(self, 'worker_seed') else 3407  # 你可以根据需要修改 worker_seed 的默认值
        seed = int(worker_seed) + int(worker_id)  # 确保每个 worker 的种子不同
        np.random.seed(seed)
        random.seed(seed)
    ...
    def train_dataloader(self):
        return DataLoader(self.dataset_train, 
                          ...
                          shuffle=True,  # 通常训练时需要 shuffle
                          num_workers=self.num_workers, 
                          worker_init_fn=self.worker_init_fn)  # 正确传递 worker_init_fn

    def val_dataloader(self):
        return (
            DataLoader(self.dataset_val, 
                       batch_size=self.batch_size, 
                       shuffle=False, 
                       num_workers=self.num_workers, 
                       worker_init_fn=self.worker_init_fn),
            DataLoader(self.dataset_test, 
                       batch_size=self.batch_size, 
                       shuffle=False, 
                       num_workers=self.num_workers, 
                       worker_init_fn=self.worker_init_fn)
