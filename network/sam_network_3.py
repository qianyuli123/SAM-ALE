...
class PromptSAMLateFusion(nn.Module):
    def __init__(
        ...
        if freeze_mask_decoder:
            for param in self.model.mask_decoder.parameters():
                param.requires_grad = False
        from feature_fusion.MCAM import MCAM
        self.fusion_neck = MCAM(768,120,256)
        # self.fusion_neck = nn.Sequential(
        #     # vit-b 768 vit-h 1280
        #     nn.Conv2d(
        #         768 + 120,
        #         256,
        #         kernel_size=1,
    ...
    def forward(self, images):
        ...
        if self.extra_encoder is not None:
            ...
            # print(f"ex_embed device: {ex_embed.device}")
            # print(f"image_embeddings device: {image_embeddings.device}")

            image_embeddings = self.fusion_neck(image_embeddings.reshape(image_embeddings.shape[0], -1, 64, 64),ex_embed)
