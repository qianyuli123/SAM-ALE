...
class PromptSAM(nn.Module):
    ...
    def __init__(
            ...
            no_sam=False,
            dropout_rate=0.3,  # 新增 Dropout 参数
    ):
        super().__init__()
    ...
    def forward(self, images):
        ...
        if self.extra_encoder is not None:
            ...
            if self.no_sam:
                ...
                # image_embeddings = self.dropout(image_embeddings + extra_image_embeddings)
...
class PromptSAMLateFusion(nn.Module):
    def __init__(
        ...
        if self.extra_encoder is not None:
            for param in self.extra_encoder.parameters():
                param.requires_grad = False  # 冻结 extra_encoder 的所有参数
        # 特征融合模块
        self.fusion_neck = nn.Sequential(
            nn.Conv2d(768 + 384, 384, kernel_size=3, padding=1, bias=False),
            LayerNorm2d(384),
            # nn.Dropout(dropout_rate),  # Dropout 应用在融合特征中
    ...
    def forward(self, images):
        ...
        if not self.feature_input:
            ...
            with torch.no_grad():
                image_embeddings_256 = self.model.image_encoder(images, no_neck=False)
                image_embeddings_384 = self.model.image_encoder(images, no_neck=True)
         # 禁用 extra_encoder 中的 Dropout，仅在推理时使用 eval()
        if self.extra_encoder is not None:
            self.extra_encoder.eval()  # 禁用 extra_encoder 中的 Dropout
            
        if self.extra_encoder is not None:
            ex_embed = self.extra_encoder(images)
            ex_embed = ex_embed.reshape(ex_embed.shape[0], 64, 64, -1)
            # ex_embed = ex_embed.permute(0, 2, 3, 1)
            # image_embeddings_384 = image_embeddings_384.permute(0, 2, 3, 1)
            # image_embeddings_256 = image_embeddings_256.permute(0, 2, 3, 1)
            # print(ex_embed.shape,image_embeddings_384.shape)
            # print(image_embeddings_384.shape, ex_embed.shape)# 2,64,64,768; 2,256,64,64
            image_embeddings_384 = torch.cat((image_embeddings_384, ex_embed), dim=-1)
            image_embeddings_384 = self.fusion_neck(image_embeddings_384.permute(0, 3, 1, 2))

        image_embeddings = image_embeddings_384
        pred_masks = []
        ious = []
        for embedding in image_embeddings: #zip(image_embeddings):
            sparse_embeddings, dense_embeddings = self.model.prompt_encoder(
            ...
            low_res_masks, iou_predictions = self.model.mask_decoder(
                ...
                # dense_prompt_embeddings=self.dropout(dense_embeddings),  # Dropout 应用在解码器特征
                multimask_output=True,
            )
            ...
            # print("pred_masks",len(pred_masks),pred_masks[0].shape)
            # print("ious",len(ious),ious[0].shape)
        return pred_masks, ious
