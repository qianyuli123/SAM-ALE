    ...
    def __init__(
        ...
        self.model.mask_decoder.output_hypernetworks_mlps = nn.ModuleList(
            [
                ...
                for _ in range(self.model.mask_decoder.num_mask_tokens)
            ]
        )

        self.model.mask_decoder.iou_prediction_head.layers[-1] = nn.Linear(
            prompt_dim, self.model.mask_decoder.num_mask_tokens
        )
        ...
        if dense_prompt_decoder:
            decoder_layer = nn.TransformerDecoderLayer(
                d_model=prompt_dim, nhead=8, dropout=dropout_rate
            )  # 在解码器层中添加 Dropout
    ...
    def forward(self, images):
        ...
        for embedding in image_embeddings:
            ...
            if self.dense_prompt_decoder is not None:
                ...
                dense_embeddings_gen = self.dense_prompt_decoder(
                    embedding_img, sparse_embeddings_v
                )
                dense_embeddings_gen = dense_embeddings_gen.permute(1, 0).reshape(
                    *org_shape
                )
...
class PromptSAMLateFusion(nn.Module):
    def __init__(
        ...
        from feature_fusion.CAG import CGAFusion

        self.fusion_neck = CGAFusion(768,120,256)

        # 特征融合模块
        # self.fusion_neck = nn.Sequential(
        #     nn.Conv2d(768 + 120, 256, kernel_size=3, padding=1, bias=False),
        #     LayerNorm2d(256),
    ...
    def forward(self, images):
        ...
        if self.extra_encoder is not None:
            ...
            image_embeddings = self.fusion_neck(image_embeddings.permute(0, 3, 1, 2),ex_embed)
