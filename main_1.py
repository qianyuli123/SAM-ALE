...
# 修改网络
from network.sam_network_ablation import PromptSAM, PromptSAMLateFusion
from pl_module_sam_seg_1 import SamSeg
...
def get_model(cfg):
    if cfg.model.extra_encoder is not None:
        ...
        if cfg.model.extra_encoder == 'hipt':
            ...
            # 使用vit_small模型结构
            my_vit = CustomVisionTransformer(img_size=256, patch_size=16, embed_dim=576, num_heads=8, depth=12, mlp_ratio=4.0)
            import torch
            ...
            from ssl_backbone.simple_vit import CustomVisionTransformer
            import torch
            extra_encoder = CustomVisionTransformer(img_size=1024, patch_size=16, embed_dim=768, num_heads=8, depth=12, mlp_ratio=4.0)
            ...
            # 加载保存的 encoder 权重
            extra_encoder.load_state_dict(torch.load(cfg.model.extra_checkpoint))
        elif cfg.model.extra_encoder == 'ab_resnet18':
            from ssl_backbone.modelfactory import ModelFactory
            import torch
            model_factory = ModelFactory(num_classes=2, in_channels=3)
            # 根据名称获取不同的模型实例
            extra_encoder = model_factory.get_model("unet_resnet18").encoder
            # 加载保存的 encoder 权重
            extra_encoder.load_state_dict(torch.load(cfg.model.extra_checkpoint))
        elif cfg.model.extra_encoder == 'ab_resnet34':
            from ssl_backbone.modelfactory import ModelFactory
            import torch
            model_factory = ModelFactory(num_classes=2, in_channels=3)
            # 根据名称获取不同的模型实例
            extra_encoder = model_factory.get_model("unet_resnet34").encoder
            # 加载保存的 encoder 权重
            extra_encoder.load_state_dict(torch.load(cfg.model.extra_checkpoint))
        elif cfg.model.extra_encoder == 'ab_resnet50':
            from ssl_backbone.modelfactory import ModelFactory
            import torch
            model_factory = ModelFactory(num_classes=2, in_channels=3)
            # 根据名称获取不同的模型实例
            extra_encoder = model_factory.get_model("unet_resnet50").encoder
            # 加载保存的 encoder 权重
            extra_encoder.load_state_dict(torch.load(cfg.model.extra_checkpoint))
        else:
...
def get_data_module(cfg):
    from image_mask_dataset_1 import GeneralDataModule, ImageMaskDataset, FtMaskDataset
...
if __name__ == '__main__':
    ...
    parser.add_argument('--csv', type=str, default="dataset_cfg/CRAG_label5%.csv")
