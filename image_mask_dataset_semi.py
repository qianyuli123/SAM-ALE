...
class ImageMaskDataset(Dataset):
    ...
    def __init__(
        ...
        self.data_type = data_type  # 保存数据类型，用于区分训练、验证、测试集

    def __len__(self):
        # 如果是训练集，返回原长度的两倍，否则保持不变
        if self.data_type == "train":
            return len(self.img_list) * 2
        return len(self.img_list)

    def setup(self, data_type, val_fold_id):
        if data_type not in ['train', 'val', 'test', 'train_pseudo']:
        ...
        if data_type == 'train_pseudo':
            if "CRAG" in self.dataset_csv_path:
                img_ids = [f"train_{i}" for i in range(1, 174)]
            elif "monuseg" in self.dataset_csv_path:
                img_ids = [f"train{i}" for i in range(1, 38)]
            else:
                raise ValueError("Unsupported dataset path. It must contain 'CRAG' or 'monuseg'.")

            # Generate random folds (1 to 4)
            folds = [random.randint(1, 4) for _ in img_ids]

            # Create DataFrame
            self.img_list = pd.DataFrame({"img_id": img_ids, "fold": folds})
    ...
    def __getitem__(self, i):
        # 对训练集的索引取模，确保索引有效
        if self.data_type == "train":
            i = i % len(self.img_list)
        row = self.img_list.iloc[i]
        img_id = row['img_id']
        image = read_rgb_img(os.path.join(self.dataset_root, SUB_FOLDER_IMAGE, str(img_id) + self.data_ext))
        mask = read_mask(os.path.join(self.dataset_root, SUB_FOLDER_MASK, str(img_id) + ".png"))
...
class GeneralDataModule(LightningDataModule):
    def __init__(self, common_cfg_dic, dataset_classs, cus_transforms, batch_size, num_workers):
        ...
        self.dataset_train, self.dataset_val, self.dataset_test, self.dataset_train_pseudo = self.initialize_dataset(common_cfg_dic,
                                                                                          dataset_classs,
                                                                                          cus_transforms)
    ...
    def initialize_dataset(self, common_cfg, DatasetCLS, cus_transforms):
        ...
        dataset_train_pseudo = DatasetCLS(**common_cfg, data_type="train_pseudo", augmentation=transforms_eval)
        return dataset_train, dataset_val, dataset_test, dataset_train_pseudo
    ...
    def val_dataloader(self):
        return DataLoader(self.dataset_val, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers), \
            ...
            DataLoader(self.dataset_train_pseudo, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers)
