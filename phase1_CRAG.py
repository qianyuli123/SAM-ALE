...
log_dir = "runs/phase1_CRAG"  # 指定日志文件夹路径
...
csv_path = os.path.join(data_dir, "CRAG_label10.csv")  # CSV 文件路径
batch_size = 4
num_epochs = 500
learning_rate = 1e-4
lambda_max = 10  # 一致性损失最大权重
consistency_rampup = 50 # 加热轮次
confidence_threshold_1 = 0.90  # 伪标签置信度阈值
confidence_threshold_2 = 0.95  # 伪标签置信度阈值
...
# 保存 encoder 的路径
encoder_save_path = "phase1_CRAG.pth"
...
def sigmoid_rampup(current, rampup_length):
    """
    实现 Sigmoid Ramp-up 函数，用于动态调整一致性损失权重。
...
def update_ema_variables(model, ema_model, alpha, global_step):
    ...
    """
    # 动态调整 alpha（训练初期使用平均，后期切换为 EMA）,EMA先大后小
    alpha = max(1 - 1 / (global_step + 1), alpha)
    
    # 遍历学生模型与教师模型对应的参数,EMA越大,教师模型更新越慢
...
student_model = get_model("effunet",num_classes=2).cuda(0)
teacher_model = get_model("effunet",num_classes=2).cuda(0)
...
metrics = initialize_metrics(num_classes=num_classes).cuda(0)
...
class LabeledDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # img_id = self.data.loc[idx % len(self.data), "img_id"]
...
for epoch in range(num_epochs):
    ...
    if epoch <= 120:
        # 置信度大于1，不使用伪标签，等监督模型收敛
        confidence_threshold = 1.1
    elif epoch <= 240:
        confidence_threshold = confidence_threshold_1
    elif epoch <= 360:
        confidence_threshold = confidence_threshold_2
    else:
    ...
    for batch_idx, batch in enumerate(tqdm(semi_supervised_loader, desc=f"Training Epoch {epoch + 1}/{num_epochs}")):
        images_sup, masks_sup = batch["image"][:labeled_bs].cuda(0), batch["mask"][:labeled_bs].cuda(0)
        ...
        # 无监督训练部分
        images_unsup = batch["image"][labeled_bs:].cuda(0)
        masks_unsup = batch["mask"][labeled_bs:].cuda(0)
    ...
    iou_scores = []
    with torch.no_grad():
        for batch in tqdm(val_loader, desc="Evaluating on val set"):
            images = batch["image"].cuda(0)
            true_masks = batch["mask"].long().squeeze(1).cuda(0)
    ...
    scheduler.step(avg_dice + avg_iou)  # 更新学习率
